...
void rcvWaypointsCallback(const nav_msgs::Path & wp)//get the target_pt from watpoints generator
{     
...
public:
    ...
    // circular obstacle
    bool isValid(const ob::State* state) const//ob::State a abstract definition of state.
    {   
        // We know we're working with a RealVectorStateSpace in this
        // example, so we downcast state into the specific type.
         const ob::RealVectorStateSpace::StateType* state3D =
            state->as<ob::RealVectorStateSpace::StateType>();//将state的ob::state 类型通过转换as()成state3D的类型ob::RealVectorStateSpace::StateType
        /**
        *
        *
...
void pathFinding(const Vector3d start_pt, const Vector3d target_pt)
{
    ...
    ob::StateSpacePtr space(new ob::RealVectorStateSpace(3));//3D 

    // Set the bounds of space to be in [0,1].
    ob::RealVectorBounds bounds(3);
    //xyz lower bounds
    bounds.setLow(0, - _x_size * 0.5);//_x_size is a global variable of 10.0
 ...
 //xyz upper bounds
    ...
    space->as<ob::RealVectorStateSpace>()->setBounds(bounds);//Cast this instance to a desired type. are they already the same type?
    ...
    ob::ScopedState<> start(space);
            //const ob::RealVectorStateSpace::StateType* state3D =
          //  state->as<ob::RealVectorStateSpace::StateType>();//将state的ob::state 类型通过转换as()成state3D的类型ob::RealVectorStateSpace::StateType
    //start.ScopedState[0]=start_pt[0];
    //start.ScopedState[1]=start_pt[1];
  //  start.ScopedState[2]=start_pt[2];
    std::vector<double> start_pt_std_vector(3);
    start_pt_std_vector[0]=start_pt[0];
    start_pt_std_vector[1]=start_pt[1];
    start_pt_std_vector[2]=start_pt[2];
    
    start = start_pt_std_vector;
    /**
    *
    *
    ...
    */
    ...
    std::vector<double> goal_pt_std_vector(3);//a container of datatype double
    goal_pt_std_vector[0]=target_pt[0];
    goal_pt_std_vector[1]=target_pt[1];
    goal_pt_std_vector[2]=target_pt[2];
   // goal[0]=target_pt[0];//also works
    //goal[1]=target_pt[1];
    //goal[2]=target_pt[2];
    goal=goal_pt_std_vector;
    ...
    *
    */  
ob::OptimizationObjectivePtr obj1 =getPathLengthObjective(si);
ob::OptimizationObjectivePtr obj2 =getThresholdPathLengthObj(si);    
 ...
 auto optimizingPlanner(std::make_shared<og::RRTstar>(si));
    //optimizingPlanner->opt_=obj2;
    // Set the problem instance for our planner to solve ob::PlannerStatus solved = ss.solve(1.0);
    optimizingPlanner->setProblemDefinition(pdef);
    optimizingPlanner->setup();

    // attempt to solve the planning problem within one second of   planner->ob::Planner::solve(1.0);
    // planning time
    ...
    if (solved)
    {
        ...
        og::PathGeometric* path = pdef->getSolutionPath()->as<og::PathGeometric>();
          cout<<"it has found the path "<<endl;
        ROS_INFO("it has found the path");
        ROS_WARN("it has found the path");
        ...
        for (size_t path_idx = 0; path_idx < path->getStateCount (); path_idx++)
        {
            ...
            *
            */ 
            ob::State *path_values = path->getState(path_idx);
            cout<<"ssssssssssssdjasjdsjs1: "<<path->getState(path_idx)<<endl;
            const ob::RealVectorStateSpace::StateType* path_values3D = path_values->as<ob::RealVectorStateSpace::StateType>();
            cout<<"ssssssssssssdjasjdsjs2: "<<path_values3D->values[0]<<"   "<<path_values3D->values[1]<<"   "<<path_values3D->values[2]<<endl;
            Vector3d values_3d;
            values_3d[0]=path_values3D->values[0];
            values_3d[1]=path_values3D->values[1];
            values_3d[2]=path_values3D->values[2];
            path_points.push_back(values_3d);
        }
