#include <iostream>
#include <ros/ros.h>//A header files set which can be found at ros wiki.
#include <nav_msgs/Odometry.h>//This message type contains information of odometry(ros wiki), like position and twist data.
#include <geometry_msgs/PoseStamped.h>//geometry_msgs geometry_msgs provides messages for common geometric primitives such as points, 
//vectors, and poses. These primitives are designed to provide a common data type and facilitate interoperability throughout the system.
#include <geometry_msgs/Pose.h>//point and orientation
#include <geometry_msgs/PoseArray.h>//header and point and orientation
#include <geometry_msgs/Vector3.h>// A generic vector or a vector for direction
#include <nav_msgs/Path.h>//This message type contains std_msgs/header(uint32 seqid, time stamp(sec or nanosec), string frame_id) and poses (point and quaternion) 
//with time stamp.
#include "sample_waypoints.h"//<> the system .h file "" is the user's .h file
#include <vector>
#include <deque>
...
nav_msgs::Path waypoints;//header+geometry_msgs/PoseStamped[] 
...
void publish_waypoints() {
    ...
    pub1.publish(waypoints);//give the goal point to the topic. //and the subsequent code is useless currently.
...
void publish_waypoints_vis() {
    ...
    {//add the start point into waypoints_vis
        geometry_msgs::Pose init_pose;
        init_pose = odom.pose.pose;
        poseArray.poses.push_back(init_pose);
    }

    for (auto it = waypoints.poses.begin(); it != waypoints.poses.end(); ++it) {//traversal the content in container
        geometry_msgs::Pose p;
        p = it->pose;//it is the content from container.
        poseArray.poses.push_back(p);
    }
    cout<<"publish_waypoints_vis has data as :"<<poseArray<<endl;
...
void goal_callback(const geometry_msgs::PoseStamped::ConstPtr& msg) {//const pointer 指针常量
/*    if (!is_odom_ready) {
    ...
    if (waypoint_type == string("circle")) {
        ...
        if (msg->pose.position.z >= 0) {
            ...
            geometry_msgs::PoseStamped pt = *msg;//header + pose (float xyz, float Quaternion)geometry_msgs::PoseStamped
            waypoints.poses.clear();//nav_msgs::Path waypoints(std_msgs/Header header+geometry_msgs/PoseStamped[] poses)
            //.clear() is from the std::vector
            waypoints.poses.push_back(pt);// add the goal into waypoints and waypoints is a global vatiable
            cout<<"waypoints has data as :"<<waypoints<<endl;
            publish_waypoints_vis();//publish waypoints_vis
            publish_waypoints();//publish waypoints
        } else {
